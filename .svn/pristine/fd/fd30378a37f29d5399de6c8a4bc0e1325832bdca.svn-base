<?php require_once('../../../../../premaster.php'); if (Usuario::logueado()->puede('administracion/tesoreria/egresos/orden_de_pago/editar/')) { ?>
<?php
...
try {
	...
	try {
		...
		$op = Factory::getInstance()->getOrdenDePago($datos['idOrdenDePago'], $empresa);
		if ($op->importePendiente != $op->importeTotal) {
			throw new FactoryExceptionCustomException('No se puede editar una órden de pago ya aplicada');
		}
		$datos['idCaja_S'] = $op->importePorOperacion->idCaja;
		...
		if ($empresa == '1' && $datos['retieneGanancias'] == 'S') {
			...
			$impRet = $prov->calcularRetencion($importeSujetoRet, false, $op->importeSujetoRetencion);
	...
	Html::jsonSuccess('Se editó correctamente la orden de pago', $op);
