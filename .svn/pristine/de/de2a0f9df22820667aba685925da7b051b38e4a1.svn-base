...
<script type='text/javascript'>
	$(document).ready(function(){
		tituloPrograma = 'Resumen bancario';
		cambiarModo('inicio');
	});

	function limpiarScreen(){
		$('#divResumenBancario').html('');
	}

	function buscar() {
		funciones.limpiarScreen();
		var url = '/content/administracion/cajas/resumen_bancario/buscar.php?',
			caja = '&caja=' + $('#inputBuscarCaja_selectedValue').val(),
			desde = '&desde=' + $('#inputBuscarDesde').val(),
			hasta = '&hasta=' + $('#inputBuscarHasta').val();
		funciones.load($('#divResumenBancario'), url + caja + desde + hasta, function() {
			$('#divResumenBancario').fixedHeader({target: 'table'});
		});
	}
	...
	function urlToExport(tipo){
		var url = '/content/administracion/cajas/resumen_bancario/get' + (tipo == 'xls' ? 'Xls' : 'Pdf') + '.php?',
			caja = '&caja=' + $('#inputBuscarCaja_selectedValue').val(),
			desde = '&desde=' + $('#inputBuscarDesde').val(),
			hasta = '&hasta=' + $('#inputBuscarHasta').val();
		return url + caja + desde + hasta;
	}
...
<div id='programaContenido'>
	<div id='divResumenBancarioWrapper'>
		<div id='divResumenBancario' class='w100p customScroll'>
		</div>
	</div>
</div>
<div id='programaPie'>
	<div id='filtro' class='hidden draggableDialog'>
		<div>
			...
			<input id='inputBuscarCaja' class='textbox autoSuggestBox obligatorio filtroBuscar w180' name="CajaBanco" />
		</div>
		<div>
