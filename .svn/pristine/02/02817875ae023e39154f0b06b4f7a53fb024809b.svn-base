...
<script type='text/javascript'>
	$(document).ready(function(){
		...
		tituloPrograma = 'Generación de notas de crédito (por factura)';
		cambiarModo('inicio');
	});
	...
	function refrescarFactura(puntoDeVenta, letra, numero) {
	...
	function goGenerarNcr(puntoDeVenta, nro, letra, nroComprobante) {
		...
		var url = '/content/comercial/notas_de_credito/generacion/factura/agregar.php';
			url += '?puntoDeVenta=' + puntoDeVenta + '&letra=' + letra + '&numero=' + nro + '&idCausa=' + idCausa;
		$.jPopUp.close();
		$.showLoading();
		$.postJSON(url, function(json){
			...
			switch (funciones.getJSONType(json)){
				...
				case funciones.jsonSuccess:
					...
					$.success('La nota de crédito fue creada correctamente');
					break;
			}
		});
	}
...
<div id='programaContenido' class='customScroll'>
	<div id='divNcrFactura' class='w100p customScroll acordeon h480'>
		<?php // TABLOTA ?>
	</div>
</div>
<div id='programaPie'>
