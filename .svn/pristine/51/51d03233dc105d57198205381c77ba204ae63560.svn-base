<?php require_once('../../../../premaster.php'); if (Usuario::logueado()->puede('administracion/proveedores/remitos_proveedor/agregar/')) {?>
<?php
...
try {
	...
	$remito = Factory::getInstance()->getRemitoProveedor();
	$remito->proveedor = Factory::getInstance()->getProveedor($idProveedor);
	$remito->fechaRecepcion = $fecha;
	$remito->sucursal = $ptoVenta;
	$remito->numero = $numero;

	$remitos = Factory::getInstance()->getListObject('RemitoProveedor', 'cod_proveedor = ' . Datos::objectToDB($remito->proveedor->id) . ' AND nro_compuesto_remito = ' . Datos::objectToDB($remito->nroCompuestoRemito));
	if (count($remitos) > 0) {
		throw new FactoryExceptionCustomException('El numero de remito "' . Funciones::padLeft($ptoVenta, 4, 0) . '-' . Funciones::padLeft($numero, 8, 0) . '" del proveedor "' . $remito->proveedor->id . '" ya existe en el sistema');
	}
	...
	$remito->detalle = $detalleRemitoAgrupado;
	...
	Html::jsonError('Ocurrió un error al intentar agregar el remito');
}
