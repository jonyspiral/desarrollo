...
<script type='text/javascript'>
	$(document).ready(function(){
		...
		tituloPrograma = 'Reimpresión de notas de crédito';
		cambiarModo('inicio');
	});
	...
	function buscar(idBuscar) {
		...
		var msgError = 'No hay notas de crédito con ese filtro',
		...
		table.append(
			$('<tr>').addClass('tableRow').append(
				$('<td>').addClass('bold aLeft').append(
					$('<label>').text(o.idCliente + ' - ' + o.razonSocialCliente + ' - NOTA DE CRÉDITO "' + o.letra + '" Nº ' + o.numeroComprobante)
				)
			),
			$('<tr>').addClass('tableRow').append(
	...
	function divBotones(o) {
		...
		btn1 = $('<a>').addClass('boton').attr('href', '#').attr('title', (o.empresa == '2' || o.caeObtenido == 'S' ? 'CAE ya obtenido' : 'Obtener CAE'))
						.attr('onclick', (o.empresa == '2' || o.caeObtenido == 'S' ? '' : 'caeClickNotaDeCredito(' + o.puntoDeVenta + ', ' + o.numero + ', "' + o.letra + '")'))
		...
		btn2 = $('<a>').addClass('boton').attr('href', '#').attr('title', (o.caeObtenido == 'S' ? (o.mailEnviado == 'S' ? 'Re-' : '') + 'Enviar' : 'Debe obtener el CAE'))
						.attr('onclick', (o.empresa == '2' || o.caeObtenido == 'S' ? 'mailClickNotaDeCredito(' + o.puntoDeVenta + ', ' + o.numero + ', "' + o.letra + '")' : ''))
		...
		btn3 = $('<a>').addClass('boton').attr('href', '#').attr('title', 'Imprimir')
						.attr('onclick', 'pdfClickNotaDeCredito(' + o.puntoDeVenta + ', ' + o.numero + ', "' + o.letra + '")')
	...
	function llenarPantalla(json) {
		var div = $('#divReimpresionNotasDeCredito');
        var table = $('<table>').attr('id', 'tablaNotasDeCredito').attr('class', 'registrosAlternados w100p');
	...
	function refrescarNotaDeCredito(puntoDeVenta, letra, numero, deleted) {
		...
		if (before.length < 1)
			...
			before.next().remove();
		if (!deleted) {
			var url = '/content/comercial/notas_de_credito/reimpresion/buscar.php?';
			...
			$.postJSON(url, function(json){
				switch (funciones.getJSONType(json)) {
					case funciones.jsonObject:
						if (before.length < 1) {
							$('#tablaNotasDeCredito tbody').first().prepend(returnTr(json.data[0]));
							$('#tablaNotasDeCredito tr').first().shine();
						} else {
	...
	function caeClickNotaDeCredito(puntoDeVenta, nro, letra) {
		funciones.guardar(
			funciones.controllerUrl('getCae'),
			{
			...
			function(json) {
				refrescarNotaDeCredito(json.data.puntoDeVenta, json.data.letra, json.data.nro);
			},
			function(json) {
				refrescarNotaDeCredito(json.data.puntoDeVenta, json.data.letra, json.data.nro);
	...
	function mailClickNotaDeCredito(puntoDeVenta, nro, letra) {
		var url = '/content/comercial/notas_de_credito/reimpresion/sendMail.php';
			url += '?puntoDeVenta=' + puntoDeVenta + '&letra=' + letra + '&numero=' + nro;
		ajax(url, function(json) {
			refrescarNotaDeCredito(json.data.puntoDeVenta, json.data.letra, json.data.nro);
			$.success('La notas de crédito se ha enviado correctamente');
		});
	}

	function pdfClickNotaDeCredito(puntoDeVenta, nro, letra) {
		var url = '/content/comercial/notas_de_credito/reimpresion/getPdf.php';
			url += '?puntoDeVenta=' + puntoDeVenta + '&letra=' + letra + '&numero=' + nro;
		funciones.pdfClick(url);
	}

	function borrar(puntoDeVenta, nro, letra){
		var msg = '¿Está seguro que desea borrar la notas de crédito "' + letra + '" Nº ' + nro + '?',
			url = '/content/comercial/notas_de_credito/reimpresion/borrar.php';
				...
				$.postJSON(url, armoObjetoBorrar(puntoDeVenta, nro, letra), function(json){
					...
					switch (funciones.getJSONType(json)){
						...
						case funciones.jsonError:
							...
							break;
						case funciones.jsonSuccess:
							$.success('La notas de crédito se ha borrado correctamente', function(){
								refrescarNotaDeCredito(json.data.puntoDeVenta, json.data.letra, json.data.nro, true);
							});
							break;
					}
				});
	...
	function cambiarModo(modo){
		...
		switch (modo){
			case 'inicio':
				...
				manejaRadioMail('A');
				$('#divReimpresionNotasDeCredito').html('');
				break;
			case 'buscar':
...
<div id='programaContenido' class='customScroll'>
	<div id='divReimpresionNotasDeCredito' class='w100p customScroll acordeon h480'>
		<?php // TABLOTA ?>
	</div>
</div>
<div id='programaPie'>
