...
<script type='text/javascript'>
	$(document).ready(function(){
		tituloPrograma = 'Movimiento de almacén MP';
	...
	function buscar() {
		if (!$('#inputBuscarAlmacen_selectedValue').val()) {
			$.error('Debe elegir el almacén desde el cual quiere realizar los movimientos');
			return;
		}
	...
	function hayErrorGuardar(obj){
		if (!obj.cantidadTotal) {
			return 'No puede hacer un movimiento vacío (todas las columnas están en cero)';
		}
		if (!obj.idAlmacen) {
			return 'Deberá elegir el almacén al que quiere mover la mercadería';
		}
		if (!obj.motivo) {
			return 'Deberá ingresar un motivo por el cual quiere mover la mercadería';
		}
		return false;
	}
	...
	function guardar(e){
		...
		if (!error) {
			$.confirm('¿Está seguro que desea mover ' + obj.cantidadTotal + ' pares del almacén ' + obj.idAlmacenOriginal + ' al ' + obj.idAlmacen + '?', function(r) {
				if (r == funciones.si) {
	...
	function armoObjetoGuardar(obj){
		...
		movimiento.idMaterial = funciones.padLeft(obj.data('idmaterial').toString(), 4);
		movimiento.idColor = obj.data('idcolormateriaprima');
		movimiento.motivo = $('#motivo_' + idComb).val();
		movimiento.cantidad = [];
		$('.material_' + idComb).each(function(){
			movimiento.cantidad[i] = funciones.toFloat($(this).val());
