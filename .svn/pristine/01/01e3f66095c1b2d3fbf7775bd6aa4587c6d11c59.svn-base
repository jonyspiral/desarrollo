<?php require_once('../../../../premaster.php'); if (Usuario::logueado()->puede('produccion/stock_mp/consumos/buscar/')) { ?>
<?php
...
try {
	if (!$idAlmacen) {
		throw new FactoryExceptionCustomException('Debe elegir el almacén desde el cual quiere realizar los consumos');
	}
	...
	if (!$one) {
		...
		$tabla = new HtmlTable(array('cantRows' => count($items), 'cantCols' => 5, 'class' => 'registrosAlternados', 'cellSpacing' => 1, 'width' => '100%'));
		$tabla->getRowCellArray($rows, $cells);
		$tabla->createHeaderFromArray(
			  array(
				   array('content' => 'Alm. actual', 'dataType' => 'Center', 'width' => 15, 'title' => 'Almacén actual'),
				   array('content' => 'Material', 'width' => 30),
				   array('content' => 'Stock', 'dataType' => 'Center', 'width' => 45),
		...
		foreach ($items as $item) {
			...
			$cells[$i][3]->class .= 'bold';
			$cells[$i][4]->content = '<a href="#" class="boton btnConfirmar" title="Confirmar" style="display: inline;" ';
			$cells[$i][4]->content .= 'data-idalmacen="' . $item['cod_almacen'] . '" data-idmaterial="' . $item['cod_material'] . '" data-idcolormateriaprima="' . $item['cod_color'] . '"><img src="/img/botones/25/aceptar.gif"></a>';
