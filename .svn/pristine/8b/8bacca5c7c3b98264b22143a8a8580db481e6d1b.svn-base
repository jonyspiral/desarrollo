...
<script type='text/javascript'>
	...
	function buscar() {
		if($('#inputBuscarNroCobro_selectedValue').val()){
			...
			var url = '/content/administracion/tesoreria/cheques/cobro_cheques_ventanilla/ingreso_cobro_cheques_ventanilla/buscar.php?idCobroChequeTemporal=' + $('#inputBuscarNroCobro_selectedValue').val(),
				msgError = 'El depósito número "' + $('#inputBuscarNroCobro_selectedValue').val() + '" no existe o no tiene permiso para visualizarlo.',
			...
			$.error('Debe seleccionar un número de cobro');
		}
	}
		...
		table.append(
			$('<tr>').addClass('tableRow').append(
				$('<td>').addClass('bold aLeft').append(
					$('<label>').text('COBRO DE CHEQUES POR VENTANILLA Nº ' + o.id + ' - CAJA: ' + o.nombreCaja + ' - RESPONSABLE: ' + o.nombreResponsable)
				)
			),
			$('<tr>').addClass('tableRow').append(
	...
	function borrar(){
		var msg = '¿Está seguro que desea borrar el cobro de cheques por ventanilla nro. "' + $('#inputBuscarNroCobro_selectedValue').val() + '"?',
