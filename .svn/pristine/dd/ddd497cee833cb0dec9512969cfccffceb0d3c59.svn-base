...
<script type='text/javascript'>
	$(document).ready(function(){
		tituloPrograma = 'Aplicador/desaplicador de documentos cliente';
		cambiarModo('inicio');
	...
	function buscar() {
		...
		var url = '/content/administracion/cobranzas/aplicacion/buscar.php?';
			url += 'desde=' + $('#inputBuscarDesde').val();
			url += '&hasta=' + $('#inputBuscarHasta').val();
			url += '&modo=' + $('#inputModo').val();
			url += '&cliente=' + $('#inputBuscarCliente_selectedValue').val();
				...
				if ($('#inputModo').val() == '2') {
					...
					$(json.hijas).each(function(i, item) {
						var docDebe = (item.debe.tipoDocumento ? item.debe.tipoDocumento : '-') + ' - ' +
								  (item.debe.nroComprobante ? item.debe.nroComprobante : '-') + ' - "' +
								  (item.debe.letra ? item.debe.letra : '-') + '"';
						var docHaber = (item.haber.tipoDocumento ? item.haber.tipoDocumento : '-') + ' - ' +
								  (item.haber.nroComprobante ? item.haber.nroComprobante : '-') +
					...
					$(json.haber).each(function(i, item) {
						var combinado = item.empresa + '_' + item.puntoDeVenta + '_' + item.tipoDocumento + '_' + item.nroDocumento + '_' + item.letra;
						...
						$('#tablaHaber tbody').append(
							$('<tr>').addClass('cPointer').attr('id', combinado).data('item', item)
								...
								.append($('<td>').addClass('aRight pRight10').text(item.nroComprobante ? item.nroComprobante : '-'))
	...
	function aplicar() {
		if ($('#tablaDebe tbody tr.selected').length && $('#tablaHaber tbody tr.selected').length) {
			var url = '/content/administracion/cobranzas/aplicacion/agregar.php?';
			$.showLoading();
			$.postJSON(url, armoObjetoGuardar(), function(json){
				...
				switch (funciones.getJSONType(json)){
					...
					case funciones.jsonObject:
						...
						item = json.data.debe;
						combinado = item.empresa + '_' + item.puntoDeVenta + '_' + item.tipoDocumento + '_' + item.nroDocumento + '_' + item.letra;
	...
	function aplicarAutomatico() {
		if ($('#tablaDebe tbody tr').length && $('#tablaHaber tbody tr').length) {
			var url = '/content/administracion/cobranzas/aplicacion/editar.php?';
			$.showLoading();
	...
	function desaplicar(e) {
		...
		var url = '/content/administracion/cobranzas/aplicacion/borrar.php?';
		$.showLoading();
	...
	function armoObjetoGuardar() {
		...
		return {
			debePuntoVenta: debe.puntoDeVenta,
			debeTipo: debe.tipoDocumento,
			debeNumero: debe.nroDocumento,
			debeLetra: debe.letra,
			haberPuntoVenta: haber.puntoDeVenta,
			haberTipo: haber.tipoDocumento,
			haberNumero: haber.nroDocumento,
			haberLetra: haber.letra
		};
	}
	...
	function cambiarModo(modo){
		...
		switch (modo){
			case 'inicio':
				...
				break;
			case 'buscar':
				funciones.cambiarTitulo(tituloPrograma + ' - ' + $('#inputBuscarCliente_selectedName').val());
				break;
		}
	}
</script>
...
<div id='programaContenido'>
	<div class="pantalla">
		<div id='divAplicadorWrapper' class='aplicador w80p'>
			<div class='w100p'>
				<div id="divHaber">
					<h2 class="aLeft">Haber (REC / NCR)</h2>
...
<div id='programaPie'>
	<div id='filtro' class='hidden draggableDialog'>
		<div>
			...
			<input id='inputBuscarCliente' class='textbox autoSuggestBox filtroBuscar w190 obligatorio' name='Cliente' alt='' />
		</div>
		<div>
