...
<script type='text/javascript'>
	...
	$(document).ready(function(){
		tituloPrograma = 'Generación notas de crédito';
	...
	function hayErrorGuardar(){
		...
		if ($('#inputCausa').val() == '')
			return 'Debe elegir una causa';
		if ($('#inputImporteGravado').val() > 0 && $('#inputDetalleItem').val() == '')
			return 'Debe ingresar el detalle gravado de la NCR';
		if ($('#inputImporteNoGravado').val() > 0 && $('#inputDetalleItemNoGravado').val() == '')
			return 'Debe ingresar el detalle no gravado de la NCR';
	}

	function guardar(){
		var url = '/content/comercial/notas_de_credito/generacion/generica/agregar.php?';
...
<div id='programaContenido'>
	<div id='divNcrGenericaDatos' class='fLeft pantalla'>
		<?php
			...
			$cells[1][1]->content .= '<option value="' . TiposDocumento2::ncrAjuste . '">Nota de Crédito Ajuste</option>';
			$cells[1][1]->content .= '<option value="' . TiposDocumento2::ncrComercial . '">Nota de Crédito Comercial</option>';
			$cells[1][1]->content .= '<option value="' . TiposDocumento2::ncrFinanciera . '">Nota de Crédito Financiera</option>';
			$cells[1][1]->content .= '</select>';
			$cells[2][0]->content = '<label>Causa:</label>';
			$cells[2][1]->content = '<input id="inputCausa" class="textbox autoSuggestBox obligatorio inputForm inputForm w230" name="CausaNotaDeCredito" rel="nombre" />';
			$cells[3][0]->content = '<label>Detalle gravado:</label>';
			$cells[3][1]->content = '<textarea id="inputDetalleItem" class="textbox inputForm inputForm w230" /></textarea>';
			$cells[4][0]->content = '<label>Detalle no gravado:</label>';
			$cells[4][1]->content = '<textarea id="inputDetalleItemNoGravado" class="textbox inputForm inputForm w230" /></textarea>';
			$cells[5][0]->content = '<label>Observaciones: </label>';
			$cells[5][1]->content = '<textarea id="inputObservaciones" class="textbox inputForm inputForm w230" /></textarea>';

			$tabla->create();
		?>
	</div>
	<div id='divNcrGenericaValores' class='fRight pantalla'>
		<?php
...
<div id='programaPie'>
	<div class='botonera'>
		<?php Html::echoBotonera(array('boton' => 'agregar', 'accion' => 'funciones.agregarClick();', 'permiso' => 'comercial/notas_de_credito/generacion/generica/agregar/')); ?>
