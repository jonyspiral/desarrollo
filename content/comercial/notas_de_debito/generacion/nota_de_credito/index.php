...
<script type='text/javascript'>
	$(document).ready(function(){
		...
		tituloPrograma = 'Generación de notas de débito (por nota de crédito)';
		cambiarModo('inicio');
	});
	...
	function divBotones(o) {
		...
		btn1 = $('<a>').addClass('boton').attr('href', '#').attr('title', 'Generar')
						.attr('onclick', 'goGenerarNcr(' + o.puntoDeVenta + ', ' + o.numero + ', "' + o.letra + '", ' + o.numeroComprobante + ')')
						.append($('<img>').attr('src', '/img/botones/40/ndb.gif'));
		div.append(btn1);
		return div;
	}
	...
	function refrescarNotaDeCredito(puntoDeVenta, letra, numero) {
	...
	function goGenerarNcr(puntoDeVenta, nro, letra, nroComprobante) {
		$.confirm('¿Está seguro que desea generar una nota de débito por la nota de crédito Nº ' + nroComprobante + '?', function(r){
			if (r == funciones.si) {
				funciones.guardar(funciones.controllerUrl('agregar'), {
					...
					refrescarNotaDeCredito(this.data.puntoDeVenta, this.data.letra, this.data.nro);
				});
			}
		});
	}
...
<div id='programaContenido' class='customScroll'>
	<div id='divNdbNotaDeCredito' class='w100p customScroll acordeon h480'>
		<?php // TABLOTA ?>
	</div>
</div>
<div id='programaPie'>
