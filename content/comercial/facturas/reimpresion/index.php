...
<script type='text/javascript'>
	$(document).ready(function(){
		...
		tituloPrograma = 'Reimpresión de facturas';
		cambiarModo('inicio');
	});
	...
	function buscar(idBuscar) {
		...
		var msgError = 'No hay facturas con ese filtro',
		...
		table.append(
			$('<tr>').addClass('tableRow').append(
				$('<td>').addClass('bold aLeft').append(
					$('<label>').text(o.idCliente + ' - ' + o.razonSocialCliente + ' - FACTURA "' + o.letra + '" Nº ' + o.numeroComprobante)
				)
			),
			$('<tr>').addClass('tableRow').append(
	...
	function divBotones(o) {
		...
		btn1 = $('<a>').addClass('boton').attr('href', '#').attr('title', (o.empresa == '2' || o.caeObtenido == 'S' ? 'CAE ya obtenido' : 'Obtener CAE'))
						.attr('onclick', (o.empresa == '2' || o.caeObtenido == 'S' ? '' : 'caeClickFactura(' + o.puntoDeVenta + ', ' + o.numero + ', "' + o.letra + '")'))
		...
		btn2 = $('<a>').addClass('boton').attr('href', '#').attr('title', (o.caeObtenido == 'S' ? (o.mailEnviado == 'S' ? 'Re-' : '') + 'Enviar' : 'Debe obtener el CAE'))
						.attr('onclick', (o.empresa == '2' || o.caeObtenido == 'S' ? 'mailClickFactura(' + o.puntoDeVenta + ', ' + o.numero + ', "' + o.letra + '")' : ''))
		...
		btn3 = $('<a>').addClass('boton').attr('href', '#').attr('title', 'Imprimir')
						.attr('onclick', 'pdfClickFactura(' + o.puntoDeVenta + ', ' + o.numero + ', "' + o.letra + '")')
	...
	function llenarPantalla(json) {
		var div = $('#divReimpresionFacturas');
	...
	function refrescarFactura(puntoDeVenta, letra, numero, deleted) {
		...
		if (before.length < 1)
			...
			before.next().remove();
		if (!deleted) {
			var url = '/content/comercial/facturas/reimpresion/buscar.php?';
				url += 'puntoDeVenta=' + puntoDeVenta;
				url += '&letra=' + letra;
				url += '&numeroComprobante=' + numero;
			$.postJSON(url, function(json){
				switch (funciones.getJSONType(json)) {
					case funciones.jsonObject:
						if (before.length < 1) {
							$('#tablaFacturas tbody').first().prepend(returnTr(json.data[0]));
							$('#tablaFacturas tr').first().shine();
						} else {
	...
	function caeClickFactura(puntoDeVenta, nro, letra) {
		var url = '/content/comercial/facturas/reimpresion/getCae.php';
			url += '?puntoDeVenta=' + puntoDeVenta + '&letra=' + letra + '&numero=' + nro;
		ajax(url, function(json) {
			refrescarFactura(json.data.puntoDeVenta, json.data.letra, json.data.nro);
	...
	function mailClickFactura(puntoDeVenta, nro, letra) {
		var url = '/content/comercial/facturas/reimpresion/sendMail.php';
			url += '?puntoDeVenta=' + puntoDeVenta + '&letra=' + letra + '&numero=' + nro;
		ajax(url, function(json) {
			refrescarFactura(json.data.puntoDeVenta, json.data.letra, json.data.nro);
			$.success('La factura se ha enviado correctamente');
		});
	}

	function pdfClickFactura(puntoDeVenta, nro, letra) {
	...
	function borrar(puntoDeVenta, nro, letra){
		var msg = '¿Está seguro que desea borrar la factura "' + letra + '" Nº ' + nro + '?',
			url = '/content/comercial/facturas/reimpresion/borrar.php';
				...
				$.postJSON(url, armoObjetoBorrar(puntoDeVenta, nro, letra), function(json){
					...
					switch (funciones.getJSONType(json)){
						...
						case funciones.jsonError:
							...
							break;
						case funciones.jsonSuccess:
							$.success('La factura se ha borrado correctamente', function(){
								refrescarFactura(json.data.puntoDeVenta, json.data.letra, json.data.nro, true);
							});
							break;
					}
				});
...
<div id='programaContenido' class='customScroll'>
	<div id='divReimpresionFacturas' class='w100p customScroll acordeon h480'>
		<?php // TABLOTA ?>
	</div>
</div>
<div id='programaPie'>
