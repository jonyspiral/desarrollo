...
function armoHead($tabla, $rango) {
	...
	$headerArray[] = array('content' => 'Artículo', 'width' => 30);
	for ($i = 0; $i < 8; $i++) {
...
try {
	if(empty($idAlmacen) && empty($idArticulo)) {
		throw new FactoryExceptionCustomException('Debe ingresar al menos un almacén o un artículo para realizar la búsqueda');
	}
