...
<script type='text/javascript'>
	$(document).ready(function(){
		tituloPrograma = 'Reimpresión venta de cheques';
		cambiarModo('inicio');
	});
	...
	function buscar() {
		...
		var url = '/content/administracion/tesoreria/cheques/venta_cheques/reimpresion_venta_cheques/buscar.php?';
		...
		table.append(
			$('<tr>').addClass('tableRow').append(
				$('<td>').addClass('bold aLeft').append(
					$('<label>').text('VENTA DE CHEQUES Nº ' + o.numero  + ' - CAJA: ' + o.caja + ' - OBSERVACIONES: ' + (o.observaciones ? o.observaciones : '-'))
				)
			),
			$('<tr>').addClass('tableRow').append(
	...
	function divBotones(o) {
		...
		botonEditar = $('<a>').addClass('boton').attr('href', '#').attr('title', 'Editar')
						...
						.data('vc', o)
						.click(function(e) {editarVentaCheques(e.target);}));
	...
	function refrescarVentaCheque(numero, deleted) {
	...
	function refrescarListaVentaCheques(id) {
		$('#tr_' + id).remove();
	}
	...
	function pdfClickDepositoBancario(nro) {
		var url = '/content/administracion/tesoreria/cheques/venta_cheques/reimpresion_venta_cheques/getPdf.php';
			url += '?numero=' + nro;
		funciones.pdfClick(url);
	}

	function editarVentaCheques(btn) {
		...
		llenarCamposEditarVentaCheques(depositoBancario);
	}

	function llenarCamposEditarVentaCheques(depositoBancario){
		setTimeout(function(){
	...
	function goEditarVentaCheques(btn){
		var objeto = {
			...
			url = '/content/administracion/tesoreria/cheques/venta_cheques/reimpresion_venta_cheques/editar.php';
