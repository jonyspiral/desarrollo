...
<script type='text/javascript'>
	$(document).ready(function(){
		tituloPrograma = 'Aplicador/desaplicador de documentos gastos';
		cambiarModo('inicio');
	...
	function buscar() {
		...
		var url = '/content/administracion/tesoreria/gastos/aplicacion/buscar.php?';
					...
					$(json.hijas).each(function(i, item) {
						...
						$('#tablaDesaplicar tbody').append(
							$('<tr>').attr('id', item.id)
								...
								.append($('<td>').addClass('aLeft').text((item.proveedor ? item.proveedor : '-')))
	...
	function aplicar() {
		if ($('#tablaDebe tbody tr.selected').length && $('#tablaHaber tbody tr.selected').length) {
			var url = '/content/administracion/tesoreria/gastos/aplicacion/agregar.php?';
			$.showLoading();
	...
	function aplicarAutomatico() {
		if ($('#tablaDebe tbody tr').length && $('#tablaHaber tbody tr').length) {
			var url = '/content/administracion/tesoreria/gastos/aplicacion/editar.php?';
			$.showLoading();
	...
	function desaplicar(e) {
		...
		var url = '/content/administracion/tesoreria/gastos/aplicacion/borrar.php?';
		$.showLoading();
...
<div id='programaContenido'>
	<div class="pantalla">
		<div id='divAplicadorWrapper' class='aplicador w80p'>
			<div class='w100p'>
				<div id="divHaber">
					<h2 class="aLeft">Haber (REND / NCR)</h2>
		...
		<div id="divDesaplicadorWrapper" class='desaplicador w100p customScroll'>
			<div>
				<table id="tablaDesaplicar" class="registrosAlternados w100p">
					<thead class="tableHeader">
						<tr class="tableRow">
							<th class="w10p cornerL5">Fecha debe</th>
							<th class="w15p bLeftWhite">Documento debe</th>
							<th class="w40p bLeftWhite">Proveedor</th>
							<th class="w10p bLeftWhite">Fecha haber</th>
							<th class="w10p bLeftWhite">Documento haber</th>
							<th class="w10p bLeftWhite">Importe aplicado</th>
