...
class Ecommerce_Core_Route {
	...
	public function __construct($path, $translation = null, $case_sensitive = null, $strip_extension = null) {
		...
		$this->case_sensitive = ($case_sensitive === null) ? true : $case_sensitive;
		$this->strip_extension = ($strip_extension === null) ? true : $strip_extension;
	}
	 ...
	 *
	 * @param Ecommerce_Core_Request $request
	 *
	 * @return    array
	 */
	public function parse(Ecommerce_Core_Request $request) {
		$uri = $request->uri->get();
		...
		if ($this->translation instanceof Closure) {
			...
			if ($uri != '') {
				if ($this->strip_extension) {
					strpos($uri, $ext = Ecommerce_Core_Input::extension()) === false or $uri = substr($uri, 0, -(strlen($ext) + 1));
				}
