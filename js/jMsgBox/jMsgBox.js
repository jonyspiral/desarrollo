	...
	$.extend({
		MsgBoxObject: {
			...
			addevents: function () {
				...
				this.esqueleto.msgbox.bind('focusout', $.proxy(function (a) {
					//Con esta función, cuando el foco sale de alguno de los elementos del msgbox, vuelve al último botón (generalmente "cancelar")
					setTimeout($.proxy(function(){
						var newFocus = $(document.activeElement);
						if ($(this.esqueleto.msgbox).find(newFocus).length == 0){
							if ((typeof newFocus[0] != 'undefined') && (typeof $(this.esqueleto.msgbox).find('button:last')[0] != 'undefined') && (newFocus[0].offsetTop < $(this.esqueleto.msgbox).find('button:last')[0].offsetTop))
								$(this.esqueleto.msgbox).find('a:visible, input:visible:enabled, button:visible:enabled').eq(0).focus();
							else
								$(this.esqueleto.msgbox).find('button:last').focus();
						}
					}, this), 1);
				}, this));
			...
			show: function (g, h, j) {
				var k = ['alert', 'info', 'error', 'prompt', 'confirm', 'success'];
			...
			toArguments: function (b) {
				return $.map(b, function (a) {
					if ($(a).val() == '')	//Esto lo agregué yo para que cuando apretan aceptar en el
						return true;		//prompt sin ingresar caracteres devuelva true.
					return $(a).val();
				});
			},
			moveBox: function () {
				...
				var c = this.esqueleto.msgbox.outerHeight(true);
				var y = 0;
				var x = 0;
		...
		success: function (a, c) {
			try {
				return $.msgbox(a, {type: 'success'}, c);
			} catch (e) {
				alert(a);
				return c();
			}
		...
		prompt: function (a, c, b) {
			try {
				if (typeof b === 'undefined')
					b = {
						type: 'prompt',
						buttons: [{type: "submit", value: 'Aceptar'}, {type: "cancel", value: 'Cancelar'}]
					};
				else
					b[type] = 'prompt';
				return $.msgbox(a, b, c);
