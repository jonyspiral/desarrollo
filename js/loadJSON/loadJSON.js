            ...
            switch (type) {
                case 'radio':
                    if (value.toString().toLowerCase() == element.value.toLowerCase())
                    	if ($(element).parent().hasClass('customRadio'))
                    		$(element).radioClick();
                    	else
                ...
                case 'text':
                case 'hidden':
                	if ($(element).hasClass('autoSuggestBox'))
        ...
        function browseJSON(obj, element, name) {
            ...
            // branch
                ...
                for (var prop in obj) {
                    ...
                    var child = jQuery.makeArray(jQuery('[rel="' + prop + '"]', element)).length > 0 ? jQuery('[rel="' + prop + '"]', element) :
                    								jQuery("." + prop, element).length > 0 ? jQuery("." + prop, element) :
                                                    jQuery("#" + prop, element).length > 0 ? jQuery("#" + prop, element) :
                                                    jQuery('[name="' + prop + '"]');
                    if (child.length != 0)
                    	if (jQuery(child, element).hasClass('autoSuggestBox')) {
                    		if (child.length == 1)
                    			setElementValue(jQuery(child, element)[0], obj[prop], prop);
                    	} else
            ...
            // data only
                ...
                if (element.length > 0) {
                    for (i = 0; i < element.length; i++)
                        setElementValue(element[i], value, name);
                }
                else {
